...
<div style="max-width:620px;margin:0 auto;padding:28px 16px;"><div style="background:#fff;border-radius:8px;padding:28px;">
    ...
    <p style="margin:0 0 20px;color:#666;">{{ $start->format('F j, Y') }} &ndash; {{ $end->format('F j, Y') }}</p>
    <p style="margin:0 0 16px;line-height:1.6;">The complete report is attached as an Excel spreadsheet. Filter dropdowns are enabled for record type, date, country, state, city, postal code, area, IP address, and the other columns.</p>
    ...
    <p style="margin:16px 0 0;color:#777;font-size:12px;">Location data &copy; OpenStreetMap contributors.</p>
</div></div>
</body></html>
